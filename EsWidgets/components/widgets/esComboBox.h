...
class ES_EXPORT EsComboItem
{
public:
    /**
     *
     * @param text item文本
     * @param icon str | QIcon | EsIconBase* | Es::IconName
     * @param userData 自定义数据
     * @param isEnabled 是否启用item
     */
    EsComboItem(const QString& text, const QVariant& icon={}, const QVariant& userData = {}, bool isEnabled = true);
    EsComboItem(const QString& text, Es::IconName icon, const QVariant& userData = {}, bool isEnabled = true);
    EsComboItem(const QString& text, const QString& icon, const QVariant& userData = {}, bool isEnabled = true);
    EsComboItem(const QString& text, const QIcon& icon, const QVariant& userData = {}, bool isEnabled = true);
    EsComboItem(const QString& text, EsIconBase* icon, const QVariant& userData = {}, bool isEnabled = true);

    QIcon getIcon() const;

    void setIcon(const QVariant& ico);
...
class EsComboBoxPrivate;

class ES_EXPORT EsComboBox : public QPushButton
{
    Q_OBJECT
    E_Q_CREATE_PIMPL(EsComboBox)
Q_SIGNALS:
...
public:
    explicit EsComboBox(QWidget* parent = nullptr);
    /**
     * @brief 添加一项
     *
     * @param text 项目文本
     ...
     * @brief 下拉菜单关闭事件处理
     *
     * 当下拉菜单关闭时，如果鼠标不在组合框区域内，则释放 dropMenu 对象。
     */
